    ...
    while range_count <= stop:  # as long as end position of iteration is less than stop position
        ...
        try:
            ...
            with lock:
                ...
                # update old range count so we can iterate over 50 next points
        ...
        # loop through in parallell index0 has altitude0
        for a, i in zip(altitudes, indexes):
...
def runner(data):
    ...
    datatot = pd.concat([data1, data2, data3, data4, data5, data6])
...
def check_wrong_altitudes(data):
    ...
    while i < len(data):
        ...
        if data.at[i, 'altitude'] > float(1441):
            print('this row has altitude higher than 1441 masl.: ',
                  data.iloc[[i]])
        i += 1
